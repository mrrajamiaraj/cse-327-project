#!/usr/bin/env python3
"""
Fix chart data to properly distribute orders across time periods
"""
import os
import django
...
def fix_chart_data_distribution():
    print("🔧 Fixing Chart Data Distribution...")
    print("=" * 50)
    ...
    try:
        ...
        print("\n📅 Creating today's orders by hour...")
        ...
        for hour, (count, min_amount, max_amount) in hourly_distribution.items():
            for i in range(count):
                # Create order time for this hour
                order_time = datetime.combine(today, datetime.min.time().replace(hour=hour)) + timedelta(
                ...
                orders_created += 1
                print(f"   {hour:2d}:00 - Order ৳{total_amount}")
        ...
        for day in range(1, today.day):  # Exclude today
            ...
            for i in range(daily_orders):
                order_time = datetime(today.year, today.month, day) + timedelta(
        ...
        for month in range(1, today.month):  # Previous months this year
            ...
            for i in range(month_orders):
                ...
                order_time = datetime(today.year, month, day) + timedelta(
                    hours=random.randint(8, 22),
                    minutes=random.randint(0, 59)
                )
        ...
        print(f"\n📊 Chart will now properly show:")
        print(f"   Daily: Real hourly distribution throughout today")
        print(f"   Monthly: Real daily distribution throughout this month")
        print(f"   Yearly: Real monthly distribution throughout this year")
